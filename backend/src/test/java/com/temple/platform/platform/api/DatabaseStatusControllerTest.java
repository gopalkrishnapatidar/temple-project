...
class DatabaseStatusControllerTest {
    ...
    void databaseReturnsSchemaAndFlywayVersions() throws Exception {
        when(applicationMetadataRepository.findValue("schema_version")).thenReturn(Optional.of("3"));
        when(applicationMetadataRepository.findLatestFlywayVersion()).thenReturn(Optional.of("3"));

        mockMvc.perform(get("/api/v1/system/database"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.schemaVersion").value("3"))
                .andExpect(jsonPath("$.flywayVersion").value("3"));
    }
}
